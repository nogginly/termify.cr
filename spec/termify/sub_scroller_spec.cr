...
Spectator.describe Termify::SubScroller do
  describe "#initialize" do
    it "clamps a height below the minimum" do
      scroller = Termify::SubScroller.new(ScriptedTerminal.at_row(1), 1)
      expect(scroller.lines).to eq(3)
    end

    it "clamps a height above the maximum" do
      scroller = Termify::SubScroller.new(ScriptedTerminal.at_row(1), 99)
      expect(scroller.lines).to eq(10)
    end

    it "is not active before starting" do
      scroller = Termify::SubScroller.new(ScriptedTerminal.at_row(1), 5)
      expect(scroller.active?).to be_false
    end
  end
  ...
  describe "#start" do
    it "reserves the region, queries the row, then confines scrolling to it" do
      ...
      scroller = Termify::SubScroller.new(ScriptedTerminal.at_row(12, written), 3)
    ...
    it "becomes active" do
      scroller = Termify::SubScroller.new(ScriptedTerminal.at_row(12), 3)
      scroller.start
      expect(scroller.active?).to be_true
    end
    ...
    it "falls back to the default row when the reply is unintelligible" do
      ...
      scroller = Termify::SubScroller.new(term, 3)
